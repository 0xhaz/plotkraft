...
- [ ] **Fountain vs PDF ingest** — RESOLVED and built (Aug 31). PDF ingest reads structure from page geometry: sluglines and action at the document's own action margin, dialogue ~1" further in, character cues ~2.2". The margin is anchored on sluglines rather than the statistical mode, because a dialogue-heavy script would otherwise set the baseline an inch too far right and reclassify every cue. 18 tests. A scan with no text layer is rejected with a clear message rather than producing an empty project.
- [ ] **Demo sample scripts — DECISION CHANGED, RISK ACCEPTED BY OWNER (Aug 31).** The plan and design.md both record "original content only — no third-party IP in the video", taken from the submission rules. The owner has decided to use real produced screenplays in the repo and demo instead, to show the canvas at feature scale and compare structural complexity across films. Recorded here because the exposure is concrete and time-sensitive: distributing full screenplays from a public repo and hosted URL invites a takedown *during judging*, which would fail the submission outright regardless of the app's quality. Mitigation if wanted later without losing the demo value: public-domain adaptations plus an original feature-length sample give the same scale-and-contrast comparison with nothing to take down.
